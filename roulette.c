...
int red[18] = {32, 19, 21, 25, 34, 27, 36, 30, 23,
               5,  16, 1,  14, 9,  18, 7,  12, 3};
int black[18] = {15, 4,  2,  17, 6,  13, 11, 8,  10,
                 24, 33, 20, 31, 22, 29, 28, 35, 26};

struct Number numbers[] = {
    // Value / color / dozen / column
    {0, 'G', 0},  {1, 'R', 1},  {2, 'B', 2},  {3, 'R', 3},  {4, 'B', 1},
    {5, 'R', 2},  {6, 'B', 3},  {7, 'R', 1},  {8, 'B', 2},  {9, 'R', 3},
    {10, 'B', 1}, {11, 'B', 2}, {12, 'R', 3}, {13, 'B', 1}, {14, 'R', 2},
    {15, 'B', 3}, {16, 'R', 1}, {17, 'B', 2}, {18, 'R', 3}, {19, 'B', 1},
    {20, 'B', 2}, {21, 'R', 3}, {22, 'B', 1}, {23, 'R', 2}, {24, 'B', 3},
    {25, 'R', 1}, {26, 'B', 2}, {27, 'R', 3}, {28, 'R', 1}, {29, 'B', 2},
    {30, 'R', 3}, {31, 'B', 1}, {32, 'R', 2}, {33, 'B', 3}, {35, 'R', 1},
    {35, 'B', 2}, {36, 'R', 3},
};
...
void rand_close(void) { fclose(urandom); }
    ...
    for (i = 0; i < num_bets; i++) {
        ...
        if (strcmp(bet.type, "number") == 0) {
            if (bet.value == number_drawn) {
                return 36.0 *
                       bet.amount; // You only win on zero when you explicitly
                                   // bet on it since zero is considered neither
                                   // ood or even nor black or red
            }
        }

        if (strcmp(bet.type, "color") == 0) {
            if ((is_black(&number) && bet.value == 0) ||
                (is_red(&number) && bet.value == 1)) {
                payout += 2.0 * bet.amount;
            }
        } else if (strcmp(bet.type, "even") == 0) {
            if ((is_even(&number) && bet.value == 1) ||
                (!is_even(&number) && bet.value == 0)) {
                payout += 2.0 * bet.amount;
            }
        } else if (strcmp(bet.type, "range") == 0) {
            if (bet.value == 0 && number_drawn >= 1 && number_drawn <= 18) {
                payout += 2.0 * bet.amount;
            } else if (bet.value == 1 && number_drawn >= 19 &&
                       number_drawn <= 36) {
                payout += 2.0 * bet.amount;
            }
        }
